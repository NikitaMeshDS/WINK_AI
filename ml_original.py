...
def process_text(text: str, episode: str) -> List[Dict]:
    ...
    if not scenes:
        logger.warning("Сцены не найдены в тексте")
        return []
    logger.info(f"Найдено сцен для обработки: {len(scenes)}")
    results = []
    if BATCH_SIZE > 1 and len(scenes) > 1 and tokenizer is not None:
        logger.info(f"Используется батчинг: размер батча {BATCH_SIZE}, всего батчей: {(len(scenes) + BATCH_SIZE - 1) // BATCH_SIZE}")
        ...
        for i in range(0, len(scenes_data), BATCH_SIZE):
            ...
            try:
                ...
                for j, (scene, _, _) in enumerate(batch):
                    ...
                    print(ensure_full_structure(result))
                    results.append(ensure_full_structure(result))
        ...
        for scene in scenes:
            ...
            try:
                ...
                result.update(rules_result)
                results.append(ensure_full_structure(result))
    logger.info(f"Обработка завершена: обработано {len(results)} сцен")
    return results
